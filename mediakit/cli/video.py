"""`mediakit video` — split, shrink, trim, lossless merge.

Primary commands:
  shrink   整片压缩为单个小视频（默认 720p）
  split    按大小或时长切分（--size / --duration / --profile）
  trim     裁剪一个子片段
  merge    无损重组（严格 -c copy）
  verify   逐帧校验
  info     查看视频信息
  caps     查看本机 ffmpeg 能力

Aliases kept for muscle memory: size, duration, social, share.
"""
...
from mediakit.core.pipeline import ConsoleReporter, Plan
from mediakit.video.capabilities import detect
from mediakit.video.encode import EncodeOptions
from mediakit.video.inputs import resolve_inputs
from mediakit.video.merge import plan_merge
from mediakit.video.probe import ProbeError, format_size, probe
from mediakit.video.split import plan_duration, plan_shrink, plan_size, plan_trim
from mediakit.video.verify import verify_concat
...
def _add_encode_args(p: argparse.ArgumentParser) -> None:
    ...
    g.add_argument(
        ...
        default="auto",
        help="编码器：auto 优先硬件(videotoolbox)，快",
    )
    g.add_argument(
        "--resolution",
        "-r",
    ...
    g.add_argument("--bitrate", type=int, default=None, help="目标视频码率 (kbps)")
    g.add_argument(
        "--audio-bitrate", type=int, default=128, help="音频码率 (kbps)，默认 128"
    )
    g.add_argument("--audio-copy", action="store_true", help="直接复制音频流")
    g.add_argument(
        "--hdr",
        choices=["auto", "sdr", "keep"],
        default="auto",
...
def _add_subcommands(sub: argparse._SubParsersAction) -> None:
    ...
    p_info.add_argument(
        "--json", action="store_true", help="以 JSON 输出（便于脚本 / 管道处理）"
    )
    ...
    p_split = sub.add_parser(
        "split",
        help="按大小或时长切分（--size / --duration / --profile）",
    )
    _add_common(p_split)
    p_split.add_argument(
        ...
        type=float,
        default=None,
        help="每段目标大小 (MB)；默认转码，可 --lossless",
    )
    p_split.add_argument(
        "--duration",
        dest="seconds",
        type=float,
        default=None,
        help="每段时长（秒）；默认无损 -c copy，可 --transcode",
    )
    p_split.add_argument(
        "--profile",
        choices=["social", "share"],
        default=None,
        help="social：≤30s 1080p H.264；share：≤200MB 1080p H.264",
    )
    p_split.add_argument(
        "--lossless",
        action="store_true",
        help="按大小切分时无损（-c copy）",
    )
    p_split.add_argument(
        "--transcode",
        action="store_true",
    ...
    _add_encode_args(p_split)

    p_size = sub.add_parser("size", help="按目标大小切分（默认 200MB；别名 split --size）")
    _add_common(p_size)
    p_size.add_argument(
        "--target-mb", "-m", type=float, default=200, help="每段目标大小 (MB)，默认 200"
    )
    p_size.add_argument(
        "--lossless", action="store_true", help="无损按大小切分（-c copy）"
    )
    _add_encode_args(p_size)

    p_dur = sub.add_parser("duration", help="按时长切分（别名 split --duration）")
    _add_common(p_dur)
    p_dur.add_argument("--seconds", "-s", type=float, required=True, help="每段时长（秒）")
    p_dur.add_argument(
        "--transcode", action="store_true", help="转码切分（默认无损 -c copy）"
    )
    ...
    p_trim.add_argument("--from", dest="start", type=float, required=True, help="起点（秒）")
    p_trim.add_argument(
        "--to", dest="end", type=float, default=None, help="终点（秒），省略则到片尾"
    )
    p_trim.add_argument(
        "--output",
        "-o",
    ...
    p_trim.add_argument(
        "--transcode", action="store_true", help="转码裁剪（默认无损 -c copy）"
    )
    ...
    p_merge.add_argument(
        "inputs", nargs="+", help="片段文件（按顺序），或一个包含片段的目录"
    )
    p_merge.add_argument(
        "--output",
        "-o",
    ...
    p_merge.add_argument(
        "--verify", action="store_true", help="合并后逐帧校验无损（需完整解码，较慢）"
    )
    _add_execution_flags(p_merge)

    p_verify = sub.add_parser(
        "verify", help="逐帧校验：整体文件 == 若干片段按序拼接"
    )
    p_verify.add_argument("whole", help="整体文件（合并输出，或切分前的源）")
    p_verify.add_argument(
        "parts", nargs="+", help="片段文件（按顺序），或一个包含片段的目录"
    )

    p_social = sub.add_parser("social", help="预设：社交短片 ≤30s 1080p H.264")
    _add_common(p_social)
    p_social.add_argument("--seconds", "-s", type=float, default=30, help="每段时长，默认 30 秒")
...
def build_parser(prog: str = "mediakit video") -> argparse.ArgumentParser:
    """Standalone parser used by tests and the vclip compatibility entry."""
    parser = argparse.ArgumentParser(
        prog=prog,
        description="把长视频切成多个小片段，整片压缩，或把片段无损拼回一个视频",
    )
...
def _plan_split(args: argparse.Namespace, info, caps):
    """Resolve `split --size/--duration/--profile` into a size or duration plan."""
    profile = getattr(args, "profile", None)
    seconds = getattr(args, "seconds", None)
    target_mb = getattr(args, "target_mb", None)
    modes = [x is not None for x in (profile, seconds, target_mb)]
    if sum(modes) != 1:
        raise ValueError(
            "split 需要且仅需要其中一个：--duration <秒>、--size <MB> 或 --profile social|share"
        )
    ...
    if profile == "social":
        ...
        return plan_duration(
            info, 30.0, caps, opts, transcode=True, outdir=args.outdir, jobs=args.jobs
        )
    if profile == "share":
        if args.resolution is None:
            args.resolution = "1080p"
        ...
        opts = _opts_from_args(args)
        return plan_size(
            info, 200.0, caps, opts, lossless=False, outdir=args.outdir, jobs=args.jobs
        )
    if seconds is not None:
        ...
        return plan_duration(
            ...
            caps,
            opts,
            transcode=bool(getattr(args, "transcode", False)),
            outdir=args.outdir,
            jobs=args.jobs,
        )
    opts = _opts_from_args(args)
...
def run(args: argparse.Namespace) -> int:
    try:
        ...
        if args.command == "merge":
            ...
            verify_fn = None
            if args.verify:
                parts = [item.path for item in plan.inputs]
        ...
        if args.command == "split":
            plan = _plan_split(args, info, caps)
